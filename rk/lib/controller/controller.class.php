...
abstract class controller {
	...
	protected 
		...
		$minifier,
		$path,
		$IGNORES_FOR_INCLUDE_DIR = array ('.', '..', 'vendor'),
		$MIN_FILE_NAME,
		$TMP_FILE_PATH,
		$TMP_FILE_NAME;
	...
	function generateCache() {
		...
		if($this->minifier == 'minify') {
			...
			$cmd = 'cd ' . \rk\manager::getRkDir() . '/vendor/minify-2.1.7/min_extras/cli && ' . $phpCmd . ' minify.php -t ' . $this->extension . ' -o  "' . $this->getCacheFilePath() . '" "' . $this->TMP_FILE_PATH . '" && chmod 775 "' . $this->getCacheFilePath() . '"';			
		} elseif($this->minifier == 'yui-compressor') {
			// launch the yui compressor to get a minified version of files
			$cmd = 'cd ' . \rk\manager::getRkDir() . '/binaries && java -jar yuicompressor-2.4.8.jar "' . $this->TMP_FILE_PATH . '" -o  "' . $this->getCacheFilePath() . '" && chmod 775 "' . $this->getCacheFilePath() . '"';
		}
