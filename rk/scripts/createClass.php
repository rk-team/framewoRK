...
include('rk/scripts/tools/createClass/parser.php');
include('rk/scripts/tools/createClass/parserMysql.php');
include('rk/scripts/tools/createClass/parserPgsql.php');
...
class createClass {
	...
	public static $allowedTypes = array('model', 'form', 'pager', 'table', 'object'),
				  ...
				  $tplPath = 'rk/scripts/tools/createClass/templates/';
	...
	public function execute() {
		...
		foreach($this->db as $oneDB) {
			...
			new \rk\model\reference\i18n(\'i18n\', \'' . $pk . '\', \'' . $modelName . '\', array(
				...
				//    I18N     //
				/////////////////
				$i18nPath = \rk\manager::getRessourcesDir() . 'i18n/';
				if($oneDB != 'default') {
					$i18nPath .= $oneDB . '/';
				}
	...
	function getFileName($dbName, $obj, $type, $base = false) {
		...
		$finalFileName = 'lib/' . $type . '/connector/';
		if($base) {
			$finalFileName .= '_base/';
		}
